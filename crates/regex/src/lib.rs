...
impl Default for NFAEntry {
    fn default() -> Self {
        unsafe {
            let mut t: [MaybeUninit<Vec<usize>>; 256] = MaybeUninit::uninit().assume_init();
            for x in &mut t {
...
impl NFA {
    ...
    fn thompson(&mut self, ast: &AST, q: usize) -> usize {
        use crate::AST::*;
        match ast {
            E => {
    ...
    fn determinization(&self) -> DFA {
        ...
        let mut tbl: Vec<DFAEntry> =
            vec![DFAEntry::default().with_accepting(s.contains(self.final_state()))];
        let mut v = vec![s];
        for i in 0.. {
            if i >= v.len() {
                break;
            ...
            for x in 0..=255u8 {
                ...
                if !u.is_empty() {
                    let j = v.iter().position(|w| w == &u).unwrap_or_else(|| {
                        tbl.push(
                            DFAEntry::default().with_accepting(u.contains(self.final_state())),
                        );
                        v.push(u);
                        v.len() - 1
                    });
...
mod tests {
                    ...
                    Box::new(K(Box::new(C(
                        ...
                        Box::new(C(
                            Box::new(C(
                                ...
                                Box::new(K(Box::new(C(Box::new(L(b'0')), Box::new(L(b'0')))))),
                            )),
                            Box::new(L(b'0')),
                        )),
                    )))),
    ...
    fn nfa_determinization_simple() {
        let ast = ast_simple();
        let dfa = NFA::new(&ast).determinization();
    ...
    fn nfa_from_dfa() {
        let ast = ast_simple();
        let nfa = NFA::from(NFA::new(&ast).determinization());
        assert!(nfa.accept("".as_bytes()));
        assert!(nfa.accept("aaab".as_bytes()));
        assert!(!nfa.accept("c".as_bytes()));
        assert!(!nfa.accept("abab".as_bytes()));
